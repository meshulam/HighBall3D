...
public class ForceArrow {
	...
	public ForceArrow(GameWorld world, float marbleRadius) {
		...
		TextureManager.getInstance().addTexture(FORCE_TEXTURE, new Texture(32, 32, new RGBColor(0, 255, 0)));
		...
		//shaft.setRotationPivot(new SimpleVector(0, -marbleRadius, 0));
	...
	public void updateArrow(final SimpleVector translation, final SimpleVector force) {
		if (force.length() < 0.1) {
			...
			Log.i("ForceArrow", "magnitude: "+force.length());

			shaft.clearTranslation();
			shaft.translate(translation);
			
			//Log.i("arrow", "position:"+position+" force:"+force+" point:"+points[1]);
